...
enum TextProcessing {
    ...
    static func removeJapaneseSpaces(_ text: String) -> String {
        var result = text
        var prev = ""
        var iterations = 0
    ...
    static func requiresClipboard(for text: String) -> Bool {
    ...
    static func contentEquals(_ a: String, _ b: String) -> Bool {
        return stripPunct(a) == stripPunct(b)
    }

    /// Compute delta (new text) between previously typed text and new transcription.
    static func computeDelta(previousText: String, newText: String) -> String {
